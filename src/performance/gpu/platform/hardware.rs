...
impl Hardware {
    ...
    pub fn new() -> Option<Self> {
        match Self::load_and_apply_configs() {
            ...
            Err(e) => {
                ...
                Some(Self { min_tdp: 0.0, max_tdp: 0.0, max_boost: 0.0 })
            }
        }
    }
    ...
    fn load_and_apply_configs() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let merged_config = Self::merge_configs(vec![
            amd_config, 
            intel_config, 
            dmi_overrides_config
        ]);
        
        log::info!("Merged configuration contains {} model configs", merged_config.models.len());
        
        // Create default configuration
        let mut hardware = Self { min_tdp: 0.0, max_tdp: 0.0, max_boost: 0.0 };
        ...
        for model in &merged_config.models {
            if model.model_name == current_model {
                ...
                log::info!("Applied configuration for model {}: min_tdp={}, max_tdp={}, max_boost={}", 
                    model.model_name, model.min_tdp, model.max_tdp, model.max_boost);
                break;
            }
        }
    ...
    fn load_config_file(file_path: &Path) -> Result<Config, Box<dyn std::error::Error>> {
        match fs::read_to_string(file_path) {
            Ok(config_str) => {
                ...
                log::info!("Loaded configuration file: {:?}, containing {} models", file_path, config.models.len());
                Ok(config)
            },
    ...
    fn get_current_model() -> Result<String, Box<dyn std::error::Error>> {
        ...
        if product_name_path.exists() {
            let model = fs::read_to_string(product_name_path)?
                .trim()
                .to_string();
            ...
            if Self::check_model_exists(&model)? {
                ...
                log::info!("No matching configuration found for product name: {}", model);
            }
        }
        ...
        if product_name_path.exists() {
            let model = fs::read_to_string(product_name_path)?
                .trim()
                .to_string();
            log::warn!("No matching configuration found, using product name as fallback: {}", model);
            return Ok(model);
        }
